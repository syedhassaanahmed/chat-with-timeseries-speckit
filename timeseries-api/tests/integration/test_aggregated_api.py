...
def test_get_aggregated_response_format() -> None:
    """Test that response matches expected schema format."""
    response = client.get(
        "/wells/WELL-001/data/aggregated",
        params={
            "metric_name": "wellhead_pressure",
    ...
    assert set(metadata.keys()) == required_metadata_keys
    ...
    if data["data"]:
        ...
        assert set(point.keys()) == required_point_keys
